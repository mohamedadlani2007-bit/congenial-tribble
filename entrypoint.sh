...
sed -e "s|__VLESS_UUID__|$VLESS_UUID|g" -e "s|__XRAY_PORT__|$XRAY_PORT|g" -e "s|__WS_PATH__|$WS_PATH|g" /etc/xray/config.template.json > /tmp/base.json
sed -e "s|__PORT__|$PORT|g" -e "s|__XRAY_PORT__|$XRAY_PORT|g" -e "s|__WS_PATH__|$WS_PATH|g" /etc/nginx/nginx.conf > /tmp/nginx.conf
export XRAY_BASE_CONFIG=/tmp/base.json XRAY_LIVE_CONFIG=/tmp/xray-live.json XRAY_PIDFILE=/tmp/xray.pid
python3 /app/bot.py & BOT_PID=$!
nginx -c /tmp/nginx.conf -g 'daemon off;' & NGINX_PID=$!
trap 'kill "$BOT_PID" "$NGINX_PID" 2>/dev/null || true' INT TERM EXIT
sleep 2
kill -0 "$BOT_PID" "$NGINX_PID"
wait "$NGINX_PID"
